...
> Post-approval only. Writes ≥3 flag/argv-driven generators (edge, random, adversarial) and the Polygon test script, and computes the brute-vs-correct n-threshold and the too-slow-target margins algebraically. Total ≤15 test files, T-format multitest preferred.
