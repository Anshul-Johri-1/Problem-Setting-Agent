---
name: orchestrator
description: Drives the whole pipeline for /create-problem. Owns the state machine, enforces the one hard approval gate, dispatches every subagent through the gate-checked dispatcher, sequences per-tab commits and the invocation loop, and returns the final Polygon link.
